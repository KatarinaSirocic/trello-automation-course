describe("Create boards + assert data", () => {
  it("Create 4 boards", () => {
    cy.intercept("GET", "/api/boards", { fixture: "boards.json" }).as(
      "create4boards"
    );
    cy.visit("/");
    //assertations - get elements trough UI
    cy.get("div:nth-of-type(1) > .board_item > .board_title").should(
      "contain",
      "IN PROGRESS"
    );
    cy.get("div:nth-of-type(2) > div:nth-of-type(1) > .board_title").should(
      "contain",
      "TO DO"
    );
  ...
  it("Assert QA Board", () => {
    cy.intercept("GET", "/api/boards/40950404759", {
      fixture: "qaBoard.json",
    }).as("qaBoard");
    cy.visit("/");
    ...
    cy.get("@qaBoard").then((res) => {
      ...
      expect(res.response.body.tasks[0].title).to.eq(
        "Stubbing network responses"
      );
      expect(res.response.body.tasks[1].title).to.eq(
        "Changing parts of response data"
      );
